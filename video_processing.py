...
def draw_parking_overlay(frame, space_status, timestamp=None):
    free_count = 0
    for area_id, area_points in PARKING_AREAS.items():
        pts = np.array(area_points, np.int32).reshape((-1, 1, 2))
        if space_status[area_id]:
            color = (0, 0, 255)  # Red for occupied
        else:
            color = (0, 255, 0)  # Green for free
            free_count += 1
        cv2.polylines(frame, [pts], isClosed=True, color=color, thickness=3)
        cv2.putText(frame, str(area_id), tuple(pts[0][0]), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
    # Draw free space count at top left
    cv2.putText(frame, f"Free: {free_count}", (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (0, 255, 0), 3)
    # Draw timestamp at top right if provided
    if timestamp:
        text_size = cv2.getTextSize(f"Time: {timestamp}", cv2.FONT_HERSHEY_SIMPLEX, 1, 2)[0]
        x = frame.shape[1] - text_size[0] - 20
        y = 40
        cv2.putText(frame, f"Time: {timestamp}", (x, y), cv2.FONT_HERSHEY_SIMPLEX, 1, (255,255,255), 2)
    return frame